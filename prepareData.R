####################################################################
###
### Data Preparation using IQB Bildungstrends 2015 as data basis
### 07.03.2019
### Sabine Zinn
###
####################################################################
...
# -------------------------------------------------------------------
# Load data ("IQB Bildungstrendsdaten 2015")
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# Check data and recoding: Data from student questionnaire 
# -------------------------------------------------------------------
...
nam[!(nam %in% names(S0))]
S <- S0[, nam]
S <- S[S$totwgt_deu>0,] # Take out cases without (German) competence measured since those have no survey weight (N=244 NAs & N=3462 Zeros)
...
S$schulart[S$schulart=="Schule mit mehreren Bildungsg?ngen"] <- "MB"
S$schulart[S$schulart=="Gymnasium"] <- "GY"
S$schulart[S$schulart=="F?rderschule"] <- "FS"

# Recode variable mapping school system 
...
# Recode variables on languages and special needs
...
# Take our variables on test exclusion / leaving test (there is nobody in the data) 
table(S$TR_LEFT, exclude=NULL) # okay
table(S$TR_EXCLUSION, exclude=NULL) 
...
table(my, exclude=NULL) # N=245 unknown
my <- as.numeric(my)
S$CMC <- 12*(my-1900)+mm
S <- S[,-which(colnames(S) %in% c("TR_MONTH", "TR_YEAR_FDZ"))]

# Recode educational attainment of mother
...
table(S$Eabildm, exclude=NULL)
S$Eabildm[S$Eabildm == names(table(S$Eabildm, exclude=NULL))[2]] <- "FHBA" 

# Recode educational attainment of father
...
table(S$Eabildv, exclude=NULL)
S$Eabildv[S$Eabildv == names(table(S$Eabildv, exclude=NULL))[2]] <- "FHBA"
...
# Have a look at students who jumped over or had to repeat a grade
table(S$Suebsp_h, exclude=NULL) # only very few students gave infos on jumping over a grade, we find N=97 of 33080: 0,3%
table(S$Swied_i, exclude=NULL) # for repeaters there are more info
S$Swied_i <- as.character(S$Swied_i)
S$Swied_i[S$Swied_i=="nicht angekreuzt"] <- 0
S$Swied_i[S$Swied_i=="angekreuzt"] <- 1
S$Swied_i[is.na(S$Swied_i)] <- 2 # Quite some missings on variable mapping repeaters -> take variable out
S <- S[,-which(colnames(S) %in% c("Swied_i", "Suebsp_h"))]

# Recode Special needs (and type of special needs)
...
# Grade in German
table(S$trnotedeu_r, exclude=NULL) # Missing for N=2813 cases, refers to ~8.5% of the sample
...
# Variables on motivation of student (item batterie, 4 items)
...
# -------------------------------------------------------------------
# Check data and recoding: Data from school heads
# -------------------------------------------------------------------
H <- md.pattern(S, plot=FALSE) 
round(H[nrow(H),]*100/nrow(S),2)

# variables on school contexts
H <- H0[, c("IDSCH_FDZ", "Pstu9jg_FDZ", "Pstudeu", "Pewsort" , "Pgtb", "Pspfges")]
table(H$Pstu9jg_FDZ, exclude = NULL) # number of 9th grade students in a school
H$Pstu9jg_FDZ <- as.numeric(as.character(H$Pstu9jg_FDZ))
table(H$Pstudeu, exclude=NULL) # proportion of students with non-German native language at school
...
H$Pewsort <- as.character(H$Pewsort) # number of people living at community where school is located
...
# -------------------------------------------------------------------
# Merge data on students with infos reported from school heads 
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# Check data and recoding: Data from teacher questionnaire
# -------------------------------------------------------------------
# data on teachers (gender, age, German teacher teaches German, lateral entrants)
...
table(L$Llbfb_a, exclude=NULL) # teacher teaches subject German (in 2015).
L$Llbfb_a <- as.character(L$Llbfb_a)
table(L$Lquest, exclude=NULL)
table(is.na(SE_$idteach_d_FDZ1)) # N=23321 students with teacherID, N=8273 without teacher ID -> Check who are these N=8273 students without teacher ID
MM <- SE_[is.na(SE_$idteach_d_FDZ1),] # Select those with tests in German/English (which are actually all), having competence tests (wle scores) in all domains of German
length(unique(MM$IDSCH_FDZ)) # Take out special need schools, concerns 396 schools of 1425, i.e. 28% of all schools and 26% of all students.
FB <- read.dta13("bt2015_lehrer_fort.dta")
FB$fort_diag[is.na(FB$fort_diag)] <- 2
table(FB$idteach_FDZ %in% L$idteach_FDZ) # All in, ok
L <- merge(L, FB[, c("idteach_FDZ", "fort_diag")], by="idteach_FDZ", all.x = TRUE)
L$fort_diag[is.na(L$fort_diag)] <- 2
L$querfremd <- ifelse(L$Lfremd_deu == 1 | L$Lquer_deu == 1, 1, ifelse(L$Lfremd_deu == 0 & L$Lquer_deu == 0, 0, 2)) # we habe 56% missingsg in teacher info on lateral entrants
...
# -------------------------------------------------------------------
# Compute and put aggregates / sum measures at class/school level
# -------------------------------------------------------------------
...
SE_$TR_geschlecht <- ifelse(SE_$TR_geschlecht == "m?nnlich", 0,1)
...
# ------------------------------------------------------------------------------------------
# Impute missing data on student level (using multivariate imputation by chained equations)
# ------------------------------------------------------------------------------------------
...
imp1$loggedEvents # Everything is fine with the imputation and the imputed values.

# -------------------------------------------------------------------
# Determine correlation between grades and competences
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# Nonresponse models for missing teacher info
# -------------------------------------------------------------------
...
for(mm in 1:imp1$m){
  ...
  IT <- merge(IT, AA, by.x="IDSCH_FDZ", by.y="SCH") # Note, here SPF students from the whole grade are included, i.e. share concerns on the one hand all children in the class without SPF (learning & emot. development) and then all children with SPF in the whole grade.
  ...
  NN1 <- unique(SL[SL$Llja_FDZ<0,"idteach_d_FDZ1"]) # Kick out data lines with missing teacher info 
  ...
  length(unique(c(NN1,NN2,NN3,NN4))) # Kick out 73 teacher (5.9%) because of miss infos. Remain N=21813 students; I loose further 1508 students due to this (6.4%).
  table(SL$idteach_d_FDZ1 %in% unique(c(NN1,NN2,NN3,NN4)))
  SL_ <- SL[!(SL$idteach_d_FDZ1 %in% unique(c(NN1,NN2,NN3,NN4))),] # Corresponds to 69.0% of the original sample (of students). 
  
  IT$Linfo <- ifelse(IT$idteach_d_FDZ1 %in% SL_$idteach_d_FDZ1, 1, 0) # N=9560 cases get lost
...
mean(rsquares)
# Get one result by pooling (using combining rules by Rubin)
...
sEff <- unique(sEff) # find sign. effects for nonresponse adjustment 

# -------------------------------------------------------------------
# Nonresponse adjustment for deriving proper survey weights, 
# Compute aggregates at class level 
...
for(mm in 1:imp1$m){
  ...
  IT <- merge(IT, AA, by.x="IDSCH_FDZ", by.y="SCH")
  IT$GY <- ifelse(IT$RS == 0 & IT$HS == 0 & IT$MB == 0 & IT$GS == 0,1,0)
  # Factor analysis for students' willingness to exert effort
  ...
  SL <- merge(IT, L, by.x ="idteach_d_FDZ1", by.y="idteach_FDZ") # N=23321 of N=31594 students, i.e. N=8273 students without teacher info (corresponds to 26.2% of the students).
  SL <- SL[,-which(colnames(SL) %in% c("IDSCH_FDZ.y", "Lquest"))]
  NN1 <- unique(SL[SL$Llja_FDZ<0,"idteach_d_FDZ1"]) # Kick out data lines with missing teacher info.
  NN2 <- unique(SL[SL$Lgender<0,"idteach_d_FDZ1"])
  NN3 <- unique(SL[SL$Lage_FDZ<0,"idteach_d_FDZ1"])
  NN4 <- unique(SL[SL$querfremd==2, "idteach_d_FDZ1"])
  length(unique(c(NN1,NN2,NN3,NN4))) # Kick out 73 teacher (5,9%) because of miss. data; remain N=21813 students; loose further N=1508 students (6.4%)
  table(SL$idteach_d_FDZ1 %in% unique(c(NN1,NN2,NN3,NN4)))
  SL_ <- SL[!(SL$idteach_d_FDZ1 %in% unique(c(NN1,NN2,NN3,NN4))),] # Corresponds to 69.0% of the original sample. 
  
  IT$Linfo <- ifelse(IT$idteach_d_FDZ1 %in% SL_$idteach_d_FDZ1, 1, 0) # We loose N=9560 cases.
